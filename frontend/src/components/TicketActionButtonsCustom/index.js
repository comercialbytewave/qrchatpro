...
import StoreClientModal from "../StoreClientModal";
import CustomerContactModal from "../CustomerContactModal";
import CartDrawer from "../ShoppCarDrawer";
...
const TicketActionButtonsCustom = ({
  ...
  const handleOpenCustomerModal = (contactId) => {
    fetchData();
  ...
  const handleOpenStoreModal = () => {
    const pedido = JSON.parse(localStorage.getItem(`pedido_${ticket.id}`));

    if (pedido) {
      // Pedido já existe: abre o carrinho
      setCartDrawerOpen(true);
    } else {
      // Não existe: abre o modal de seleção de loja
  ...
  const handleDeletePedido = () => {
    localStorage.removeItem(`pedido_${ticket.id}`);
  };

  const handleStoreModalComplete = (pedido) => {
    console.log("Pedido criado:", pedido);
    // Aqui você pode navegar, atualizar estado, etc.
  };
          ...
          <>
            ...
            <>
              ...
              <CustomerContactModal
                ...
                contactId={contactId}
                reload={handleStoreModalComplete}
              />
              <StoreClientModal
                open={storeModalOpen}
                onClose={handleCloseStoreModal}
                ticket={ticket}
                onComplete={handleStoreModalComplete}
              />
              <CartDrawer
                open={cartDrawerOpen}
                ...
                onCheckout={(pedidoFinalizado) => {
                  // Callback quando finaliza a compra
              ...
              {!ticket?.contact?.customer && (
              <IconButton className={classes.bottomButtonVisibilityIcon}>
                <Tooltip title="Cadastro Clientes">
                  <Wc
                    // color="primary"
                    onClick={e => handleOpenCustomerModal(ticket?.contact?.id)}
                  />
                </Tooltip>
              </IconButton> )}

              <IconButton className={classes.bottomButtonVisibilityIcon}>
                <Tooltip title="Pedido Venda">
                  <ShoppingCart
                    // color="primary"
